//----------------------------------------------------------------------------
// $Id: SolverDB.hpp 1426 2008-06-04 21:18:22Z broderic $
//----------------------------------------------------------------------------
...
public:
    ...
    /** Opens (or creates if it does not already exist) a SolverDB for
        ...
        of stones for transposition states.  Returns true if open was
        successful, and false otherwise (i.e. db already exists but
        with different settings). */
    bool open(int width, int height, int maxstones, int transtones,
              const std::string& filename);
    
    /** Opens an existing database for the given boardsize. Returns
        false if db does not exist or if it exists but with a
        different boardsize. */
    bool open(int width, int height, const std::string& filename);
