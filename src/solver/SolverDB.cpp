//----------------------------------------------------------------------------
// $Id: SolverDB.cpp 1715 2008-10-28 22:13:37Z broderic $
//----------------------------------------------------------------------------
...
bool SolverDB::get(const StoneBoard& brd, SolvedState& state)
{
    ...
    if (0 < count && count <= m_settings.maxstones) {
        ...
        if (m_db.Get(rotated_brd.Hash(), state)) {
            ...
            state.proof = brd.rotate(state.proof);
            state.winners_stones = brd.rotate(state.winners_stones);
...
int SolverDB::write(const StoneBoard& brd, const SolvedState& state)
{
    ...
    if (0 < count && count <= m_settings.maxstones) {
        ...
        // do not overwrite a small proof with a larger one
...
int SolverDB::put(const StoneBoard& brd, const SolvedState& state)
{
    ...
    if (0 < count && count <= m_settings.maxstones) {
	    ...
	    if (hex::settings.get_bool("solverdb-store-trans"))
		wrote += SolverDBUtil::StoreTranspositions(*this, brd, state);
	    if (hex::settings.get_bool("solverdb-store-flips"))
                                     ...
                                     const SolvedState& state)
{
    ...
    bitset_t flippedBlack = brd.mirror(brd.getWhite() & brd.getPlayed() &
				       brd.getCells());
    bitset_t flippedWhite = brd.mirror(brd.getBlack() & brd.getPlayed() &
				       brd.getCells());
    ...
    bitset_t flippedProof = brd.mirror(state.proof);
