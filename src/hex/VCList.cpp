//----------------------------------------------------------------------------
// $Id: VCList.cpp 1536 2008-07-09 22:47:27Z broderic $
//----------------------------------------------------------------------------
...
VCList::VCList(HexPoint x, HexPoint y, unsigned int soft, ChangeLog<VC>& log)
    : m_x(x), m_y(y), 
      m_softlimit(soft),
      m_vcs(),
      m_log(log),
...
bool VCList::isVCSuperSetOfAny(const VC& vc) const
{
    const_iterator it, end = m_vcs.end();
    for (it = m_vcs.begin(); it != end; ++it) {
        if (BitsetUtil::IsSubsetOf(it->carrier(), vc.carrier()))
            return true;
    }
    return false;
}

int VCList::removeSuperSetsOf(const VC& vc, bool dirty_intersections)
{
    int count = 0;
    for (iterator it = m_vcs.begin(); it != m_vcs.end(); ) {
        if (BitsetUtil::IsSubsetOf(vc.carrier(), it->carrier())) {
...
{
    iterator inserted_at;
    return add(vc, m_vcs.begin(), inserted_at);
}
                              ...
                              iterator& inserted_at)
{
    ...
    logAdd(vc);
    it = inserted_at = m_vcs.insert(it, vc);
...
int VCList::add(const VCList& other)
{
    ...
    /** @note we try to do this efficiently by tracking the position
        of the last added vc and starting the next add() from
        there.  */
    iterator start = m_vcs.begin();
    for (const_iterator it = other.begin(); it != other.end(); ++it) {
        ...
        iterator inserted_at;
        if (this->add(v, start, inserted_at)) {
            start = inserted_at;
            count++;
        }
    }
...
/** @bug Not used? Why not!? */
int VCList::add(const VCList& other, const VCList& check)
{
    int count = 0;
    iterator start = m_vcs.begin();
    for (const_iterator it = other.begin(); it != other.end(); ++it) {
        if (!check.isVCSuperSetOfAny(*it)) {
            VC v(getX(), getY(), it->key(), it->carrier(), 
                 it->stones(), it->rule());
            v.setProcessed(false);
...
VCList::iterator VCList::remove(iterator it)
{
    logRemove(*it);
    it = m_vcs.erase(it);
