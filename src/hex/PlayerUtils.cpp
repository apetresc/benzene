//----------------------------------------------------------------------------
// $Id: PlayerUtils.cpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
...
HexPoint PlayWonGame(const HexBoard& brd, HexColor color)
{
    ...
    if (brd.Cons(color).doesVCExist(HexPointUtil::colorEdge1(color),
                                     HexPointUtil::colorEdge2(color),
                                     VC::FULL))
    {
        hex::log << hex::fine << "Winning VC." << hex::endl;
        std::vector<VC> vcs;
        brd.Cons(color).getVCs(HexPointUtil::colorEdge1(color),
                                HexPointUtil::colorEdge2(color),
                                VC::FULL, vcs);
...
HexPoint PlayLostGame(const HexBoard& brd, HexColor color)
{
    ...
    */
    bool connected = brd.Cons(other).doesVCExist(otheredge1, otheredge2, 
                                                 VC::SEMI);
    std::vector<VC> vcs;
    brd.Cons(other).getVCs(otheredge1, otheredge2, 
