//----------------------------------------------------------------------------
// $Id: GroupBoard.hpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
...
public:
    ...
    GroupBoard(unsigned size);

    /** Constructs a rectangular board. */
    GroupBoard(unsigned width, unsigned height);

    /** Constructs a GroupBoard from the given StoneBoard. */
    GroupBoard(const StoneBoard& brd);
    ...
    /** Computes neighbours of all empty cells, going through groups
        of color.  Neighbours of groups of color are also included in
        nbs. */
    void ComputeDigraph(HexColor color, PointToBitset& nbs) const;
    ...
    /** Clears absorbed information; sets all cells to be in their own
        group. */
    void clearAbsorb();
...
protected:
    ...
    /** Called in StoneBoard when the board is modified. */
