//----------------------------------------------------------------------------
// $Id: VCUtils.cpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
                              ...
                              HexPoint& edge)
{
    ...
    if (!brd.isAdjacent(miai[0], miai[1])) 
        return false;

    // find the two cells adjacent to both
    std::vector<HexPoint> adj;
    for (BoardIterator n1(brd.const_nbs(miai[0])); n1; ++n1)
        for (BoardIterator n2(brd.const_nbs(miai[1])); n2; ++n2)
...
bool isBridge(const StoneBoard& brd, HexColor color, const VC& vc)
{
    ...
    for (BoardIterator it1 = brd.const_nbs(miai[0]); it1; ++it1)  
    {
	for (BoardIterator it2 = brd.const_nbs(miai[1]); it2; ++it2) 
        {
	    if (*it1 == *it2) 
            {
		commonNbs.set(*it1);
                                ...
                                const VC& vc)
{
    ...
    {
        edge_stones[i]
            = BoardUtils::ReachableOnBitset(brd, brd.getColor(color), 
                                            EMPTY_BITSET, edge[i]);
    }
    ...
    for (int i=0; i<2; ++i) {
        ...
        reachable[i] 
            = BoardUtils::ReachableOnBitset(brd, our_carrier, 
                                            EMPTY_BITSET, edge[i]);
...
bool ValidMaintainable(const HexBoard& brd, HexColor color, const VC& vc)
{
    ...
    if (vc.rule() == VC_RULE_AND)
    {
        for (BoardIterator g(brd.Groups(color)); g; ++g) {
            ...
            VC vc1, vc2;
            if (brd.Cons(color).getSmallestVC(vc.x(), *g, VC::FULL, vc1) &&
                brd.Cons(color).getSmallestVC(vc.y(), *g, VC::FULL, vc2)) 
            {
                                  ...
                                  std::size_t max)
{
	...
	for (BoardIterator g2(brd.Groups(c)); *g2 != *g1; ++g2) 
	    ...
	    if (maintain.size() < max 
                && brd.Cons(c).getSmallestVC(*g1, *g2, VC::FULL, vc)
