//----------------------------------------------------------------------------
// $Id: Resistance.cpp 1699 2008-09-24 23:55:56Z broderic $
//----------------------------------------------------------------------------
                          ...
                          AdjacencyGraph graph[BLACK_AND_WHITE])
{
    ...
    if (hex::settings.get_bool("resist-simulate-and-over-edge"))
                 ...
                 AdjacencyGraph& graph)
{
    ...
    for (BoardIterator x(brd.Stones(not_other)); x; ++x) 
    {
        for (BoardIterator y(brd.Stones(not_other)); *y != *x; ++y) 
        {
            ...
            if ((cx==cy) || brd.Cons(color).doesVCExist(cx, cy, VC::FULL))
            {
                graph[*x][*y] = true;
                graph[*y][*x] = true;
            }
...
void InitializeCapMiai()
{
    ...
    hex::log << hex::fine << "--InitializeCapMiai" << hex::endl;
                         ...
                         HexPoint edge, AdjacencyGraph& graph)
{
    ...
    for (BitsetIterator p(augment); p; ++p) {
        for (BoardIterator q(brd.const_locations()); q; ++q) {
...
{
    if (hex::settings.get_bool("vc-and-over-edge")) {
        hex::log << hex::warning 
                 << "**** \"resist-simulate-and-over-edge\" is being used "
                 << "while \"vc-and-over-edge\" is on!! ****" 
                 << hex::endl;
    }
