//----------------------------------------------------------------------------
// $Id: TwoDistance.cpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
...
void TwoDistance::ComputeScores(HexColor color, HexEval* out)
{
    ...
    for (BoardIterator it(m_brd->const_cells()); it; ++it) {
...
void TwoDistance::ComputeScore()
{
    ...
    int SCALE_FACTOR = hex::settings.get_int("twod-scale-factor");
...
{
    VC vc;
    if (m_brd->Cons(color).getSmallestVC(p1, p2, VC::FULL, vc)) {
        switch(m_ntype) {
