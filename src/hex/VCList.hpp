//----------------------------------------------------------------------------
// $Id: VCList.hpp 1536 2008-07-09 22:47:27Z broderic $
//----------------------------------------------------------------------------
...
/**
   A sorted list of VCs.  We assume that each vc in the list is
   between the same pair of cells, although this is not checked or
   enforced.
   ...
   There are two size limits.  The soft limit is the number of vcs
   used in vc calculations.  VCs that appear after the soft limit are
   not considered in vc calculations, but may be later on if some vcs
   are removed.  The idea is to keep around extra vcs that may be
...
public:
    ...
    VCList(HexPoint x, HexPoint y, unsigned int soft, ChangeLog<VC>& log);
    ...
    /** Returns true if some VC's carrier in the list is a subset of
        the given VC (ignoring keys). */
    bool isVCSuperSetOfAny(const VC& vc) const;
    ...
    /** Removes any connection whose carrier is a superset of the
        ...
        member of this list!
    */
    int removeSuperSetsOf(const VC& vc,
    ...
    /** Adds vc to the list.  
        ...
        Requires a single pass over the entire list.  The add will
        fail if vc is a superset of some vc already in the list.
        Supersets of this vc that are already in the list are removed.
        If, after adding vc, the list is longer than the hardlimit, 
        the last vc in the list will be removed.

        @return true if the VC was added, false otherwise.
    */
    AddResult add(const VC& vc);


    /** Adds the elements of other to list. Returns number of vcs
        added. Vcs are added as unprocessed. */
    int add(const VCList& other);


    /** Adds the elements of other to list.  Calls add(VC) for each VC
        in other if VC is not superset of any connection in check.
        Vcs are added as unprocessed. Returns number of vcs added. */
    int add(const VCList& other, const VCList& check);
    ...
    */
    ...
    int removeAllContaining(const bitset_t& b);
...
protected:
    ...
    /** Internal add() for speed. Only use this if you know what you're
        doing! */
    AddResult add(const VC& vc, const iterator& start, iterator& inserted_at);

    /** Records in the log that this vc was added to the list. */
    void logAdd(const VC& vc);

    /** Records in the log that this vc was removed from the list. */
    void logRemove(const VC& vc);
...
inline void VCList::logAdd(const VC& vc)
{
    m_log.push(ChangeLog<VC>::ADD, vc);
}

inline void VCList::logRemove(const VC& vc)
{
    m_log.push(ChangeLog<VC>::REMOVE, vc);
}
