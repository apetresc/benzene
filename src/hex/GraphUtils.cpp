//----------------------------------------------------------------------------
// $Id: GraphUtils.cpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
                         ...
                         PointToBitset& group_nbs)
{
    ...
    for (BoardIterator i(brd.Groups(not_other)); i; ++i) {
	group_nbs[*i] = captains & brd.Cons(c).ConnectedTo(*i, VC::FULL);
    }
}
...
#if 1
    hex::log << hex::fine << "Shortest Path Frequency Info:" << hex::endl;
						       ...
						       bool preferKeys)
{
...
#if 1
    hex::log << hex::fine << "Cells on Shortest VC Paths (w/o edges):"
	     << hex::endl;
    for (BitsetIterator i(onShort); i; ++i) {
	hex::log << *i << ": " << numPaths[*i] << hex::endl;
    }
    hex::log << hex::endl;
#endif
	...
	bitset_t nbrOfI = group_nbs[*i];
	    ...
	    const VCList& lst 
                = brd.Cons(c).getVCList(*i, *j, VC::FULL);
...
#if 1
    hex::log << hex::fine << "Cells on Shortest VC Paths (with edges):"
	     << hex::endl;
						   ...
						   bitset_t cellsToOrder)
{
	...
	distFromCentre += brd.distance(*i, brd.centerPointLeft());
	distFromCentre += brd.distance(*i, brd.centerPointRight());
    ...
    hex::log << hex::info << "MOVE ORDERING" << hex::endl;
    for (unsigned i=0; i<cellEvals.size(); i++) 
    {
	hex::log << hex::info << cellEvals[i].point()
		 << ": " << cellEvals[i].value()
		 << hex::endl;
    }
}
