//----------------------------------------------------------------------------
// $Id: StoneBoard.cpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
...
{
    m_played = ConstBoard::rotate(m_played);
    for (BWIterator it; it; ++it)
        m_stones[*it] = ConstBoard::rotate(m_stones[*it]);
    computeHash();
    modified();
}
...
{
    m_played = ConstBoard::mirror(m_played);
    for (BWIterator it; it; ++it)
        m_stones[*it] = ConstBoard::mirror(m_stones[*it]);
    computeHash();
    modified();
}
...
BoardID StoneBoard::GetBoardID() const
{
    ...
    /** @note When this code was written, the cells were iterated over
        ...
        changes to the order in const_cells() will break all existing
    ...
    for (BoardIterator p(const_cells()); p; ++p, ++i) {
        val[i] = (played.test(*p)) 
...
void StoneBoard::SetState(const BoardID& id)
{
    ...
    /** @note This depends on the order defined by const_cells().
