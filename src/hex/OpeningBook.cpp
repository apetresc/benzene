//----------------------------------------------------------------------------
// $Id: OpeningBook.cpp 1657 2008-09-15 23:32:09Z broderic $
//----------------------------------------------------------------------------
...
//    : m_leafs(hex::settings.get_int("book-cache-bits"))
{
...
void OpeningBook::Expand(HexBoard& brd, int numExpansions)
{
	...
	hex::log << hex::info << "\n--Iteration " << i << "--" << hex::endl;
        ...
        int flush = hex::settings.get_int("book-flush-iterations");
        if (i && (i % flush)==0) {
                                         ...
                                         const OpeningBookNode& parent)
{
    ...
    if (!HexEvalUtil::IsWinOrLoss(value) && 
         hex::settings.get_bool("book-dampen-scores")) 
    {
...
{
    bool using_cache = hex::settings.get_bool("book-use-cache");
	...
	if (child.IsDummy()) 
        {
            ...
            if (hex::settings.get_bool("book-use-cache")) 
            {
                m_leafs[brd.Hash()] = child;
                m_statistics.cache_writes++;
            } 
