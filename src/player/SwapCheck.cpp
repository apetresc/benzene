//----------------------------------------------------------------------------
// $Id: SwapCheck.cpp 1715 2008-10-28 22:13:37Z broderic $
//----------------------------------------------------------------------------
...
SwapCheck::SwapCheck(UofAPlayer* player)
...
{
    if (hex::settings.get_bool("game-allow-swap") 
        && (1 == game_state.History().size())
        && (!FIRST_TO_PLAY == color))
    {
	HexAssert(1 == brd.numStones());
	hex::log << hex::info << "Performing swap pre-check..." << hex::endl;
	...
	if (brd.width() != brd.height()) 
		...
		(brd.width() < brd.height() && color == VERTICAL_COLOR))
            {
                hex::log << hex::info << "Non-square board: " 
		...
		firstMove = brd.mirror(firstMove);
	...
	hex::log << hex::warning << "Opted not to swap." << hex::endl;
    }
