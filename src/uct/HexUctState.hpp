//----------------------------------------------------------------------------
// $Id: HexUctState.hpp 1699 2008-09-24 23:55:56Z broderic $
//----------------------------------------------------------------------------
...
struct HexUctInitialData
{
    ...
    /** VCs that we can choose to maintain during rollouts. */
    std::vector< std::vector<VC> > maintainVCs;

    HexUctInitialData()
	: maintainVCs(BITSETSIZE)
    { }
...
private:
    ...
    /** Radius 1 means we only need to update ring godels (faster). In the
	rollout we only need radius 1, but for initialization patterns we
	may want radius > 1 in the tree. */
    int m_treeUpdateRadius;
    int m_rolloutUpdateRadius;

    /** Used to select VCs for rollout maintenance and evaluation. */
    bool m_maintainingVCs;
    int m_maintainVCBitShift;
    
    bool m_opptMaintainedVCs;
    bool m_playerMaintainedVCs;
    std::vector<HexPointPair> m_maintainedVCs;
