#include <boost/test/auto_unit_test.hpp>

#include "ChangeLog.hpp"

//---------------------------------------------------------------------------

namespace {

BOOST_AUTO_UNIT_TEST(ChangeLog_InitialState)
{
    ChangeLog<float> cl;
    BOOST_CHECK(cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 0);
}

BOOST_AUTO_UNIT_TEST(ChangeLog_Activation)
{
    ChangeLog<float> cl;
    BOOST_CHECK(!cl.activated());
    cl.activate(false);
    BOOST_CHECK(!cl.activated());
    cl.activate(true);
    BOOST_CHECK(cl.activated());
    cl.activate(false);
    BOOST_CHECK(!cl.activated());
    cl.activate(true);
    BOOST_CHECK(cl.activated());
    cl.activate(true);
    BOOST_CHECK(cl.activated());
    cl.activate(false);
    BOOST_CHECK(!cl.activated());
    cl.activate(false);
    BOOST_CHECK(!cl.activated());
}

BOOST_AUTO_UNIT_TEST(ChangeLog_PushPopTopAndClear)
{
    ChangeLog<float> cl;
    // testing push before activated
    BOOST_CHECK(cl.empty());
    BOOST_CHECK(!cl.activated());
    cl.push(ChangeLog<float>::ADD, 0.0f);
    BOOST_CHECK(cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 0);
    BOOST_CHECK(!cl.activated());
    // testing push once activated
    cl.activate(true);
    BOOST_CHECK(cl.activated());
    cl.push(ChangeLog<float>::ADD, 0.1f);
    cl.push(ChangeLog<float>::REMOVE, 0.2f);
    cl.push(ChangeLog<float>::REMOVE, 0.3f);
    cl.push(ChangeLog<float>::MARKER, 0.4f);
    BOOST_CHECK(!cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 4);
    // testing topAction and topData
    BOOST_CHECK_EQUAL(cl.topAction(), ChangeLog<float>::MARKER);
    BOOST_CHECK_EQUAL(cl.topData(), 0.4f);
    BOOST_CHECK(!cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 4);
    // testing activation's effect on push
    cl.activate(false);
    BOOST_CHECK(!cl.activated());
    cl.push(ChangeLog<float>::ADD, 0.5f);
    cl.push(ChangeLog<float>::MARKER, 0.6f);
    cl.push(ChangeLog<float>::ADD, 0.7f);
    BOOST_CHECK(!cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 4);
    // testing topAction and topData unchanged
    BOOST_CHECK_EQUAL(cl.topAction(), ChangeLog<float>::MARKER);
    BOOST_CHECK_EQUAL(cl.topData(), 0.4f);
    BOOST_CHECK_EQUAL(cl.size(), 4);
    // testing activation has no effect on pop
    BOOST_CHECK(!cl.activated());
    cl.pop();
    BOOST_CHECK(!cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 3);
    BOOST_CHECK_EQUAL(cl.topAction(), ChangeLog<float>::REMOVE);
    BOOST_CHECK_EQUAL(cl.topData(), 0.3f);
    // testing clear (Note: activation has no effect)
    BOOST_CHECK(!cl.activated());
    cl.clear();
    BOOST_CHECK(cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 0);
    BOOST_CHECK(!cl.activated());
    // test popping last item
    cl.activate(true);
    BOOST_CHECK(cl.activated());
    cl.push(ChangeLog<float>::MARKER, 1.0f);
    cl.push(ChangeLog<float>::ADD, 1.1f);
    BOOST_CHECK_EQUAL(cl.topAction(), ChangeLog<float>::ADD);
    BOOST_CHECK_EQUAL(cl.topData(), 1.1f);
    cl.push(ChangeLog<float>::REMOVE, 1.2f);
    BOOST_CHECK(!cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 3);
    cl.pop();
    cl.pop();
    BOOST_CHECK(!cl.empty());
    BOOST_CHECK_EQUAL(cl.size(), 1);
    BOOST_CHECK_EQUAL(cl.topAction(), ChangeLog<float>::MARKER);
    BOOST_CHECK_EQUAL(cl.topData(), 1.0f);
    cl.pop();
    BOOST_CHECK_EQUAL(cl.size(), 0);
    // check clear on empty changelog, ensure does not affect activation
    BOOST_CHECK(cl.empty());
    BOOST_CHECK(cl.activated());
    cl.clear();
    BOOST_CHECK(cl.empty());
    BOOST_CHECK(cl.activated());
}

BOOST_AUTO_UNIT_TEST(ChangeLog_Dump)
{
    ChangeLog<float> cl;
    BOOST_CHECK_EQUAL(cl.dump(), "");
    cl.activate(true);
    cl.push(ChangeLog<float>::ADD, 2.0f);
    cl.push(ChangeLog<float>::MARKER, 2.1f);
    cl.push(ChangeLog<float>::REMOVE, 2.2f);
    std::ostringstream s;
    s << "0:    ADD: 2" << std::endl
      << "1: MARKER" << std::endl
      << "2: REMOVE: 2.2" << std::endl;
    BOOST_CHECK_EQUAL(cl.dump(), s.str());
}

}

//---------------------------------------------------------------------------
