//----------------------------------------------------------------------------
// $Id: ChangeLog.hpp 1426 2008-06-04 21:18:22Z broderic $
//----------------------------------------------------------------------------
...
/** 
    ...
    When changelog is activated, push functions as expected, but when not
    activated, push will not do anything. This is used for static versus
    incremental VC computation (i.e. we do not wish to log anything for
    static). Note that activation does not affect any other method.
*/
template<typename T>
class ChangeLog
{
public:
    ...
    /** Returns true if changelog has been activated. */
    bool activated() const;

    /** Activates/Deactivates the log. */
...
template<typename T> 
inline bool ChangeLog<T>::activated() const
{
    return m_activated;
}

template<typename T> 
inline void ChangeLog<T>::activate(bool flag)
{
    m_activated = flag;
}
