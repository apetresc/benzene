...
/** @file GtpEngine.cpp
    ...
    @note This file copied (stolen!?) from Fuego-0.1.  
    The only change is in HandleCommand(), where if the callback is not
    found, hex::settings is checked and the value stored if the setting
    is found. 
...
bool GtpEngine::HandleCommand(GtpCommand& cmd, ostream& out)
{
    ...
    try
    {
        ...
        {
            // handle hex::setting stuff
            if (cmd.NuArg()==0 && hex::settings.defined(cmd.Name())) {
                response = cmd.Name() + " = " + hex::settings.get(cmd.Name());
            } else if (cmd.NuArg()==1 && hex::settings.defined(cmd.Name())) {
                hex::settings.put(cmd.Name(), cmd.Arg(0));
                response = cmd.Name() + " = " + hex::settings.get(cmd.Name());
            } else {
                status = false;
